...
namespace Simulation
{
	...
	void InertiaTest::OnInit(GLFWwindow* window)
	{
		...
		Graphics::Model* boxModel = new Graphics::Poly(vertices, indices);

		indices.clear();
		mesh.GetLineIndices(indices);
		static_cast<Graphics::Poly*>(boxModel)->SetFrame(vertices, indices);

		Physics::Collider* boxCollider = new Physics::HullCollider(mesh);
		boxCollider->SetPosition(glm::vec3(0));
		boxCollider->SetModel(boxModel);
		Graphics::Model* f = new Graphics::Poly(vertices, indices);
